...
export type CronField = CronFieldDef & {
  token: string;
  description: string;
};
...
function isWildcard(token: string) {
  return token === "*";
}
...
function isRangeToken(token: string) {
  return /^\d+-\d+$/.test(token);
}

function isStepToken(token: string) {
  return /^(\*|\d+-\d+|\d+)\/\d+$/.test(token);
}

function validateSingleToken(token: string, field: CronFieldDef) {
...
function weekdayText(token: string) {
...
function describeField(token: string, field: CronFieldDef): string {
  if (token === "*") {
...
function buildSummary(fields: CronField[]) {
...
export function parseCron(expression: string): { error: string; expression?: never; fields?: never; summary?: never } | { expression: string; fields: CronField[]; summary: string; error: null } {
